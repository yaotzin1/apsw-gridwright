...
> **Status**: Implemented 2026-09-25 (C-1 resolved as (b); contract in api-surface.md, review in review.md)
> **Stage entry**: 1 & 2
...
3. **No keyboard route without checkboxes.** Rows are not focusable, so removing the checkboxes
   removes the only keyboard-operable selection control. A grid nobody can operate by keyboard is a
   broken grid. Since 0.11.0 `cellNavigation()` gives the grid a focused cell, and `Space` on a cell
   holding a checkbox already operates it; what is missing is `Space` on a cell that holds none.
...
- [x] **AC-03** `selection({ selectAll: false })` renders the checkbox column's header as an empty
...
- [x] **AC-04** Row-click selection: with `selectOnRowClick: true` and a selection mode other than
...
- [x] **AC-05** Interactive element protection: a click whose target is inside `button`, `a`, `input`,
      `select`, `textarea`, `[role="button"]`, `[role="menuitem"]` or `[contenteditable]` within the row
      does not toggle selection.
- [x] **AC-06** Keyboard selection: with `selectOnRowClick: true`, `Space` on a focused body cell
      toggles the selection of its row, through the add-on's `tableKeyDown` contribution, which
      returns `true` only when it handled the key. It steps aside when the focused cell holds a
      control (`cellNavigation()` operates that control instead) and when focus is inside an
      interactive element listed in AC-05. The focused cell comes from `cellNavigation()` (C-1).
- [x] **AC-12** Selecting text is not a click: a click that ends a text selection inside the row does
      not toggle it, so a reader can copy a value out of a row-click grid.
- [x] **AC-13** A row-click grid says so to the pointer: with `selectOnRowClick` on and selection
      active, rows carry `.gw-row--selectable`, which the stylesheet gives `cursor: pointer`.
...
- [x] **AC-10** New strings are in the `gridwright:selection` add-on's messages, in five languages and in
...
> **Delivered 2026-09-25: every criterion.** A grid with `checkboxes: false` selects by row click with
> `selectOnRowClick`, and by `Space` on a focused cell when `cellNavigation()` is listed. Without
> `cellNavigation()` it has no keyboard route, and `docs/api.md` says so beside the option.
...
options. A consumer changes them through the core set:
...
- **C-1. How does a row receive focus when there are no checkboxes?** **Resolved 2026-09-25: (b).**
  `cellNavigation()` owns focus; `selection()` handles `Space` on the focused cell when that cell holds
  no control, and otherwise leaves the key to `cellNavigation()`, which operates the control. One
  focus model per table. Without `cellNavigation()`, `checkboxes: false` has no keyboard route, and
  `docs/api.md` and `docs/accessibility.md` say so next to the option. The original text follows.
  Open for stage 2. Rows are not
...
- **Does a row click compete with other row-click behaviour?** The grid's `onRowClick` runs too (the
  shell's handler first). `rowActions({ trigger: 'click' })` opens its menu on the same click; the
  combination is allowed and documented rather than prevented, because a consumer may want both.
- **Why a class for the pointer rather than styling `[aria-selected]` rows?** `aria-selected` is on
  every row of any selectable grid, including checkbox grids where a row click does nothing. The
  class marks exactly the grids where the row is the control.
